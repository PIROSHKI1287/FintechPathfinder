...
### QA エージェント起動条件（必須トリガー）

以下の**いずれか1つ**に該当する場合、QA エージェントを起動しなければならない。省略禁止。

| トリガー | 説明 |
|---|---|
| 新規 UI コンポーネント作成 | `'use client'` を含む新規ファイルが追加された場合 |
| 既存 UI の動作フロー変更 | ページ遷移・イベントハンドラ・状態管理に変更がある場合 |
| DB シード・マイグレーション | データ構造・初期データを変更した場合 |
| API エンドポイント追加・変更 | Route Handler に変更がある場合 |
| タスク完了プロトコル Step 3 | 常に E2E テスト計画を作成し人間に提示する（全タスク必須） |

### QA チェックリスト（最低限・省略禁止）

QA エージェントは以下を確認してから PASS を出すこと：

- [ ] `npm run dev` 起動でエラーなし（`.next` 削除後に再確認）
- [ ] 対象画面が初期表示でエラーなし
- [ ] 主要インタラクション（ボタン・フォーム・ページ遷移）が動作する
- [ ] ブラウザコンソールに赤エラーがない
- [ ] 人間向け E2E テスト計画（`plans/*-e2e.md`）が作成・提示済み
